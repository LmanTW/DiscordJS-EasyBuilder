class Page {
  ...
  collect (target, options) {
    ...
    if (options === undefined) {
      options = { 
        filter: (type === 'interaction') ? i => i.user.id === target.user.id && this._idUsed.includes(i.customId) : i => i => i.user.id === target.interaction.user.id && this._idUsed.includes(i.customId), 
        time: 600000, 
        max: 1 
      }
    }
